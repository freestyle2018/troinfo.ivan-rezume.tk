...
echo"В данном разделе Вы можете выбрать, интересующий Вас раздел, и увидеть отмодерировано ли Ваше объявление.<b><span style='color:black'>Модерация объявления проиcходит в течении суток с момента подачи объявления.</span></b><br><br>\n";
...
echo "   </td>\n";
echo "   <td>\n";
  echo"&nbsp; - &nbsp; Объявление ожидает модерацию; &nbsp;&nbsp;\n";
echo "   </td>\n";
echo "   <td>\n";
...
if($doska_kat_id_post <> ""){
   $zapros3=@"SELECT * FROM doska_kat WHERE doska_kat_id = ".$doska_kat_id_post."";
}                                                                                     
if($doska_kat_id_get <> ""){
   $zapros3=@"SELECT * FROM doska_kat WHERE doska_kat_id = ".$doska_kat_id_get."";
}   

$result3=@mysql_query($zapros3);
$num_results3 =@ mysql_num_rows($result3);

echo "<table><tr><td align='left'>\n";
for ($i=0; $i < $num_results3; $i++){
   $row3 = mysql_fetch_array($result3);
   echo "<h1>".$row3["doska_kat_name"]."</h1>";
}   
echo "</td></tr></table>\n";
